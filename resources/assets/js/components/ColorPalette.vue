<template>
  <div>
    <ul class="uk-accordion" uk-accordion>
      <li :class="{ 'uk-open' : color_id }"><span href="#" class="uk-accordion-title">{{ trans.color }}</span>
            ...
            <li v-for="color in palette" :uk-tooltip="color.name">
              <div v-for="colArr in colorArr">
                <div v-if="colArr == color.name ">
                      <label>
                        <input type="radio" name="palette" v-on:click="pickColor(color.name.toLowerCase())" :checked="color.name.toLowerCase() == color_id"/>
                          <lazy-background
